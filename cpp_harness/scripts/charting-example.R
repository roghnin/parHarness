...
data$rideable<-as.factor(gsub("SGLMap","SGLMap",data$rideable))
data$rideable<-as.factor(gsub("LFHashMap","LFHashMap",data$rideable))



ddply(.data=data,.(rideable,threads),mutate,ops_max= max(ops)/(interval*1000000))->data
sgldata <- subset(data,str_detect(rideable,"SGLMap"))
lfdata <- subset(data,str_detect(rideable,"LFHashMap"))
...
#names(color_key) <- unique(c(as.character(LFdata$rideable)))
...
ggsave(filename ="fastchart.eps",fastchart,width=8, height = 5.5, units = "in", dpi=300)
